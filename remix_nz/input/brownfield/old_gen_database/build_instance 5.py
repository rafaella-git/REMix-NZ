...
files_lst=["nz-hydro","nz_profile_11nodes","medpop_evs_base","low_pop_out_base","med_pop_out_base","high_pop_out_base"] 
...
def add_gas_turbines(m):    
    ...
    gt_cap = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [gt_nodes, range(1960, 2060), gt_techs]
        )
    )
...
def add_thermal(m):
    ...
    the_cap = pd.DataFrame(
        index=pd.MultiIndex.from_product(
            [the_nodes, range(1960, 2060), the_techs]
        )
    )
    ...
    for _, row in grouped_df.iterrows():
        ...
        if year_built + 30 >= 2022:
            ...
            #print(f'the_cap.loc[idx[["{node}"], [{year_built}], "{techs}"], "unitsBuild"] = {capacity}')
            exec(code)
...
def add_hydro(m):
    ...
    hydro_coef.loc[idx[:, :, "Spill", "Water_in"], "coefficient"] = -1000 # GW_el
    hydro_coef.loc[idx[:, :, "Spill", "Water_out"], "coefficient"] = 1000 # GW_el

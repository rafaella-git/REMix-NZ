...
fixsingleyear=2050
yrs2run=[2020,2025]#,2030,2040,2050] #[2050] # years to be optimised
indx=0
...
def load_feedin_csv(year, aggregate=False, norm=True):
    inst = pd.read_csv(Path(path_profiles).joinpath("region_statistics_2020.csv"), index_col=[0, 1])
...
#%% [markdown]
# ## add_gas_turbines function with NZ database 
# ### MAAM: NZ thermal 2022 database  
def add_gas_turbines_v2(m):
    gt_inst_csv = pd.read_excel(Path(path_xlsx).joinpath("power-plant-nz-database.csv"))
    ...
    # Emit carbon from combustion
    gt_emission = pd.DataFrame(index=pd.MultiIndex.from_product([["CO2_emission"], ["global"], ["GT", "CCGT","OCGT"], gt_vintage, gt_activities]))
    gt_emission.loc[idx[:, :, "GT", :, :], "perActivity"] = np.round(-1 / np.array([0.41, 0.43]), 3) * 0.2016 # kt_CO2
    gt_emission.loc[idx[:, :, "CCGT", :, :], "perActivity"] = np.round(-1 / np.array([0.58, 0.61]), 3) * 0.2016 # kt_CO2
    gt_emission.loc[idx[:, :, "OCGT", :, :], "perActivity"] = np.round(-1 / np.array([0.4695, 0.4695]), 3) * 0.2016 # kt_CO2 - data: REMix Tutorial 202, number for 2020 used for 2000 and 2030
    m.parameter.add(gt_emission, "accounting_converteractivity")

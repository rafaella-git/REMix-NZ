...
group_name="will"
will_elec = ["02-battery-overnight", "03-battery-recharging", "04-battery-solar"]#["01-battery-distributed", "02-battery-overnight", "03-battery-recharging", "04-battery-solar"]
will_h2 = ["01-h2-distributed", "02-h2-overnight", "03-h2-recharging", "04-h2-solar"]
will_lst=will_elec+will_h2 
...
def build_and_run(index):
    print(f"Building and running: {files_lst[index]} for {yrs_str}")
    ...
    def add_hydro(m):
        ...
        conv_cap = pd.DataFrame(
            index=pd.MultiIndex.from_product(
                [hydro_nodes, [2000]+yrs_to_calc, hydro_techs] 
            )
        )
        ...
        stor_res = pd.Dataframe(index=pd.MultiIndex.from_product([hydro_nodes, yrs_to_calc, stor_techs]))
    ...
    def add_lithium_batteries(m):
        ...
        conv_cap = pd.DataFrame(
            index=pd.MultiIndex.from_product(
                [battery_nodes, yrs_to_calc, battery_techs]
            )
        )
        ...
        stor_res = pd.DataFrame(
            index=pd.MultiIndex.from_product([battery_nodes, yrs_to_calc, battery_techs])
        )
    ...
    def add_h2_storage(m):
        ...
        conv_cap = pd.DataFrame(
            index=pd.MultiIndex.from_product(
                [h2_stor_nodes, yrs_to_calc, h2_stor_techs]
            )
        )
        ...
        conv_coef = pd.DataFrame(
            index=pd.MultiIndex.from_product(
                [h2_stor_techs,  yrs_to_calc, ["Charge", "Discharge"], ["H2", "H2_stored","Elec"]]
            )
        )
        # eltr_coef.loc[idx[:, :, :, "Elec"], "coefficient"] = -1
        #FIXME: takke coefs we want
        conv_coef.loc[idx[:, :, "Charge", "Elec"], "coefficient"] = [ -0.043346085,	-0.035470537, -0.031529343]
        #[ -0.043346085,-0.043346085, -0.035470537,	-0.035470537, -0.035470537, -0.031529343, -0.031529343]
        ...
        stor_res = pd.DataFrame(
            index=pd.MultiIndex.from_product([h2_stor_nodes, yrs_to_calc, h2_stor_techs])
        )
...
def multiple_cases():
    indx = 0
    while indx < len(files_lst):
        build_and_run(indx)
        indx += 1
 
# Run the function to execute the script multiple times
if __name__ == "__main__":
    multiple_cases()

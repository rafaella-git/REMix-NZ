# %% [markdown]
# # Building profile
# This script creates different demand profiles generating .csv files to be used in build_instance
...
# Select years
def get_years():
  """User chooses the number of years or enter their own list.

  Returns:
    A list containing the selected years.
  """
  options = {
      1: [2050],
      2: [2020, 2050],
      4: [2020, 2030, 2040, 2050],
      7: [2020, 2025, 2030, 2035, 2040, 2045, 2050]
  }

  print("Choose how many years to run:")
  for num_years, years in options.items():
    print(f"{num_years} ({', '.join(str(y) for y in years)})")
  print("Type 'other' to enter your own list:")

  choice = input("Enter your choice (1,2,4,7, other): ")

  if choice.isdigit() and int(choice) in options:
    return options[int(choice)]
  elif choice.lower() == "other":
    years = input("Enter your desired years separated by commas: ")
    return [int(year) for year in years.split(",")]
  else:
    print("Invalid choice. Please try again and choose 1, 2, 4, 6 or other.")
    return get_years()  # Recursively call the function if input is invalid
# Get user's choice and store the years
years = get_years()
years_len=len(years)
print(f"You selected {years_len} year(s): {years}")

# %%
# Electricity demand
def get_elec_profile():
  """USer chooses electricity demand file

  Returns:
    The name of the chosen electricity demand CSV file.
  """
  print("This run will create demand profiles for the selected years.")
  print('Using the default electricity demand projected ("nz_electricity_demand.csv").')
  choice = input("Do you want to use other demand profiles? (yes/no): ")

  if choice.lower() == "yes":
    file_name = input("Enter the name of your demand CSV file: ")
    if not file_name.endswith(".csv"):
      file_name += ".csv"  # Add ".csv" if it's missing
    return file_name
  else:
    return "nz_electricity_demand.csv"  # Keep the default file
electricity_file = get_elec_profile()
print(f"The electricity demand profile is: {electricity_file}")  

# %%
# Hydro conditions
def get_hydro_conditions():
  """User chooses hydro conditions scenario.

  Returns:
    The chosen hydro conditions scenario ("Driest years" or "Average years").
  """
  print("Choose hydro conditions scenario:")
  print("1. Driest years")
  print("2. Average years")
  ...
  if choice.isdigit() and int(choice) in {1, 2}:
    ...
    return get_hydro_conditions()  # Recursively call the function for invalid input
hydro_conditions = get_hydro_conditions()
print(f"You selected: {hydro_conditions}")
...
# demand_files=["nz-elec","nz-elec"]
# yrs=[[2050],[2020,2030,2040,2050]] # years optimised
...
# folder_lst=["nz-elec_2050","nz-elec_2020-2030-2040-2050"]
...
# caps=caps_dict(results,False,False)
# print(caps)

...
fixsingleyear=2050
yrs2run= [2020] # years to be optimised [2020,2030,2040,2050] #
indx=0
...
files_lst=["nz-hydro","nz-elec","medpop_evs","low_pop_out","med_pop_out","high_pop_out"] 
...
def add_hydro(m):
    ...
    hydro_cap = pd.DataFrame(
        index=pd.MultiIndex.from_product(
            [hydro_nodes, [2020, 2025, 2030, 2035, 2040, 2045, 2050], hydro_techs]
        )
    )
    # in the vbracket the order is [model regions years techs]
    hydro_cap.loc[idx[:, :, hydro_techs], "unitsUpperLimit"] =  5.33618 #100  # GW_el
    # existing capacities: fixz to make it automatic
    hydro_cap.loc[idx[["BOP"], [2020], "Turbine"], "unitsBuild"] = 0.17095  # GW_el
    hydro_cap.loc[idx[["CAN"], [2020], "Turbine"], "unitsBuild"] = 1.82683  # GW_el
    hydro_cap.loc[idx[["CEN"], [2020], "Turbine"], "unitsBuild"] = 0.399  # GW_el
    hydro_cap.loc[idx[["HBY"], [2020], "Turbine"], "unitsBuild"] = 0.1422  # GW_el
    hydro_cap.loc[idx[["NEL"], [2020], "Turbine"], "unitsBuild"] = 0.0453  # GW_el
    hydro_cap.loc[idx[["OTG"], [2020], "Turbine"], "unitsBuild"] = 1.664  # GW_el
    hydro_cap.loc[idx[["WTO"], [2020], "Turbine"], "unitsBuild"] = 1.0873  # GW_el
